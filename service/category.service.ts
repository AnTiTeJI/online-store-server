...
class CategoryService {
  async findCategoryByName(name: string): Promise<CategoryModel> {
    const categoryDb: CategoryModel | null = await Category.findOne({
      where: { name },
    });
    if (!categoryDb) throw ApiError.notFound("Category not found");
    return categoryDb;
  }
  ...
  async addCategories(
    ...
    categories: string[]
  ): Promise<{ msg: string; categories: string[] } | void> {
    let errors: string[] | undefined =
      ApiFunction.findDuplicateOnArray(categories);
    ...
    for (let category of categories) {
      const childCategoryDb: CategoryModel = await this.findCategoryByName(
        category
      );
      const chainCategoriesDb: string[] = await this.getAllParentCategories(
        childCategoryDb
      );
  ...
  async createCategory(
    ...
    if (parent_name) {
      ...
      if (!parentDb) throw ApiError.notFound("Category not found");
  ...
  async getAllParentCategories(
    ...
    if (category.ParentId) {
      const categoryDb: CategoryModel = await this.findCategoryById(
        category.ParentId
      );
