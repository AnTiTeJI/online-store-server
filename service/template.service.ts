...
class TemplateService {
  ...
  async CheckTemplates(
  ...
  ): Promise<boolean | { msg: string; characteristics: string[] }> {
    let chNames: string[] = characteristics.map((obj) => obj.name);
    let errors: string[] | undefined =
      ApiFunction.findDuplicateOnArray(chNames);
    ...
    for (let category of categories) {
      const childCategoryDb: CategoryModel =
        await categoryService.findCategoryByName(category);
      const parentCategoriesDb: string[] =
        await categoryService.getAllParentCategories(childCategoryDb);

      for (let parent of parentCategoriesDb) {
        const categoryDb: CategoryModel =
          await categoryService.findCategoryByName(parent);
        const templateDb: TemplateModel = await categoryDb.getTemplate();
        if (templateDb) {
          const characteristicsDb: string[] = JSON.parse(
            templateDb.characteristics
          );
