...
class BasketService {
    async findBasketById(id: number) {
        const basket = await Basket.findByPk(id);
        if (!basket)
            throw ApiError.notFound("Basket not found");
        return basket;
    }
    async addProductToBasket(basket: BasketModel, product: ProductModel) {
    ...
    async removeProductFromBasket(basket: BasketModel, product: ProductModel) {
        const productsDb = await basket.getProducts();
        for (let productDb of productsDb) {
            if (productDb.name !== product.name)
                throw ApiError.badRequest("Product already added to the basket");
    ...
    async getProductFromBasket(basket: BasketModel) {
        let products = [];
        const productsDb = await basket.getProducts();
        for (let productDb of productsDb)
    ...
    async RemoveProductsFromStock(basket: BasketModel) {
