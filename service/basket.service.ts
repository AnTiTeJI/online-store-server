...
class BasketService {
    ...
    async addProductToBasket(basket: BasketModel, product: ProductModel): Promise<void> {
    ...
    async removeProductFromBasket(basket: BasketModel, product: ProductModel): Promise<void> {
        const productsDb: ProductModel[] = await basket.getProducts();
        for (let productDb of productsDb) {
            if (productDb.name !== product.name)
                throw ApiError.badRequest("Product already added to the basket");
