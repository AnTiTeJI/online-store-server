...
class BasketController {
  ...
  async removeProductFromBasket(
    ...
    try {
      ...
      await basketService.removeProductFromBasket(
        await user.getBasket(),
        product
      );
  ...
  async getProductsFromBasket(
    ...
    try {
      ...
      const products: IProductFullBody[] =
        await basketService.getProductFromBasket(await user.getBasket());
      res.status(200).json({ products });
    } catch (error) {
      next(error);
    }
  }
  async buyProductsFromBasket(
    req: Request,
    res: Response,
    next: NextFunction
  ): Promise<void> {
