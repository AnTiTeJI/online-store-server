...
class UserController {
    ...
    async changeUserDetails(req: Request, res: Response, next: NextFunction): Promise<void> {
        try {
            console.log("Change user details");
            const user: UserModel = await userService.findUserByTokenId(req.headers.authorization);
            const newUser: UserModel = await userService.changeUserDetails(user, req.body);
            res.status(200).json({
                email: newUser.email,
