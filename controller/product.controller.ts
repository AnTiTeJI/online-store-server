...
class ProductController {
  async createProduct(
    ...
    try {
      const { product, categories, characteristics } =
        req.body as IProductFullBody;
      const errors = await templateService.CheckTemplates(
        categories,
        characteristics
      );
      if (errors) return res.status(404).json({ errors });

      const productModel: ProductModel = await productService.createProduct({
        product,
        categories,
        characteristics,
      });
  ...
  async getProduct(
    ...
    try {
      const product: ProductModel = await productService.findProductById(
        Number(req.params.id)
      );
      const characteristicsDb: CharacteristicModel[] =
      ...
      const characteristicsDto = characteristicsDb.map((ch) => {
  ...
  async getProducts(
    ...
    try {
      ...
      const products: IProductFullBody[] = await productService.getProducts(
        Number(limit),
        offset
      );
  ...
  async addProductImages(
    ...
    try {
      ...
      if (req.query.img == "main")
        for (let i in images) {
          await imageService.addProductImage(
            product,
            images[i] as UploadedFile,
            true
          );
